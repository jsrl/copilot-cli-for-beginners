"""Tests for the book collection module."""
...
def test_add_book_returns_created_book() -> None:
    collection = BookCollection()
...
def test_add_book_persists_to_data_file() -> None:
    collection = BookCollection()
...
def test_mark_book_as_read_only_marks_matching_book() -> None:
    collection = BookCollection()
    collection.add_book("Dune", "Frank Herbert", 1965)
    collection.add_book("The Hobbit", "J.R.R. Tolkien", 1937)
...
def test_remove_book_not_found_returns_feedback() -> None:
    collection = BookCollection()
    collection.add_book("Dune", "Frank Herbert", 1965)
...
def test_remove_book_case_insensitive_and_trimmed_title() -> None:
    collection = BookCollection()
    collection.add_book("Dune", "Frank Herbert", 1965)
...
def test_remove_book_returns_suggestion_for_partial_match() -> None:
    collection = BookCollection()
...
def test_remove_book_from_empty_collection() -> None:
    collection = BookCollection()
    result, message = collection.remove_book("Dune")
    assert result is False
...
def test_list_by_year_empty() -> None:
    collection = BookCollection()
    collection.add_book("1984", "George Orwell", 1949)
